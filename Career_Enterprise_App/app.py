  ...
  <div class="timeline-track">
    ...
    if degree_str and degree_str != "—":
        if st.button(f"🎓 Find Top Colleges for: {degree_str}", key=f"link_cg_{career.get('Career_Name','').replace(' ','_')}"):
            st.session_state.search_query = degree_str
    ...
    if not filtered_courses:
        ...
        for idx, course in enumerate(filtered_courses):
            ...
            if st.session_state.get(toggle_fees, False):
                ...
                st.markdown(
                    f'<div class="startup-detail" style="border-color:rgba(16,185,129,0.3); background:rgba(16,185,129,0.06); color:#34d399;">'
                    f'💰 <strong>Average Fee Structure:</strong><br>{fees_text}</div>',
                    unsafe_allow_html=True
                )
            ...
            if st.session_state.get(toggle_coach, False):
                ...
                if platforms:
                    ...
                    st.markdown(
                        f'<div class="startup-detail" style="border-color:rgba(45,212,191,0.3); background:rgba(45,212,191,0.06); color:#5eead4;">'
                        f'💻 <strong>Recommended Online Coaching:</strong><br><div style="margin-top:6px;">{platform_tags}</div></div>',
                        unsafe_allow_html=True
                    )
                else:
